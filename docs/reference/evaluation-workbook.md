...
For a new experiment, follow steps 2–10 once; for saved work, use step 2's resume guidance. The default commands do **not** assume a reviewed regression exists.
...
This is the advanced C path, not a continuation of A's worksheet or B's `outputs/candidate/`.
Introductory B's `collect/evaluate` evaluates project Responses outputs; this workbook collects answers
**actually generated by one exact Hosted agent version**, separated by model. These saved runs and their scores are not interchangeable.
...
**Name the evidence before collecting.** Record these labels in `session-notes.txt`; each later reference must identify the same experiment.

| Label | First created by | Saved evidence |
|---|---|---|
| `wf-baseline` | Step 5, v1 dev collection | `outputs/benchmarks/wf-baseline/` |
| `wf-candidate` | Step 7, v2 dev collection | `outputs/benchmarks/wf-candidate/` |
| `judge-calibration` | Step 8, correct/incorrect judge fixtures | `outputs/judge-calibration/judge-calibration/calibration.json` |
| `wf-final` | Step 9, frozen candidate on final holdout | `outputs/benchmarks/wf-final/` |

For a new experiment, use unused labels and update their `--label`, `--baseline`, `--candidate`, `--reference`,
`--holdout` and `--calibration` references consistently. When resuming, keep the original labels, packages, versions and responses;
do not repeat completed preparation, deployment or collection. Inspect the saved manifests and continue at the first unfinished stage.
An existing holdout remains the final cohort, not an invitation to collect under a new name.
A judge job still running after a timeout is resumed with the same evaluation command and label.

**Record the acceptance policy now, before seeing scores.** The default final command requires valid native evaluation,
verified traces and passing calibration; it does **not** require every native quality check for the selected candidate/holdout models to pass.
If your policy requires that too, record `--require-native-pass` and add it to both step 9's verification and the Lab 11 handoff command.
Do not add or remove it after reviewing results.
...
**Collection check — use it for baseline, candidate and holdout.**
In this run's folder (`outputs/benchmarks/wf-baseline/` here), inspect `manifest.json` and `business-evaluation.json`.
`benchmark collect` exits `1` for business failures **or request errors**. Continue to the judge only when
the manifest has `status: completed`, `actual_rows` equals `expected_rows`, and the business report has `errors: 0`.
A failed business answer with zero request errors is still valid evaluation input, not a passing candidate.
Retain failed dev answers for review. For request errors, missing rows or exit `2`, preserve the evidence,
...
Apply the [collection check](#matrix-collection-check) to `outputs/benchmarks/wf-candidate/` first.
Only then compare the frozen configuration locally before the next paid judge job:
...
Keep every final row, including failures. Apply the [collection check](#matrix-collection-check) to
`outputs/benchmarks/wf-final/` before native evaluation; a full row count with request errors does not pass that check:
...
Use the acceptance policy recorded in step 2, including `--require-native-pass` only if selected then.
Check `native_quality_required` against that choice. The default distinguishes valid native execution/lineage from native quality:
`review-native-findings` can accompany `gate_passed: true`, so read the recommendation, not just the Boolean.
Use the [Hosted handoff outcomes](../labs/11-capstone.md#hosted-acceptance) to record ready for human review, unresolved native findings, rejection or incomplete evidence.
