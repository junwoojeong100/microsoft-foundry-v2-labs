...
| `collect --split dev --label baseline --prompt v1 --retrieval local` | Paid calls for all dev cases | Preserves errors; concurrency one |
| `evaluate --label baseline` | Local evaluation report; no Azure calls | Deterministic business checks |
| `compare --baseline baseline --candidate candidate` | Local comparison report; no Azure calls | Controlled dev comparison |
...
| `accept --candidate candidate --holdout final-holdout` | Local acceptance/rejection report; no Azure calls | Human acceptance evidence, not automatic approval |
