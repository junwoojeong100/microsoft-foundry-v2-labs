...
이전 영상을 다시 편집한 것이 아닙니다. 지정된 실습 계정으로 실행했으며 기본 Azure CLI 구독은 변경하지 않았습니다.
...
이전 녹화 파일·캡처와 현재 가이드의 이전 영상 링크는 새 자료로 교체합니다.
**Git 이력은 재작성하지 않으며, GitHub 첨부 원본의 영구 삭제는 완료했다고 주장하지 않습니다.**
첨부의 영구 삭제는 GitHub 지원 경로 확인이 필요합니다.
이전 Azure 리소스와 루트의 기존 `.env`·`.azure`는 이번 작업에서 변경하지 않았습니다.

새 실행은 기존 azd 프로젝트 바깥의 독립된 폴더에서 수행했습니다.
