...
4. Tools에 기본 **Web search**가 있으면 **Actions for Web search → Remove**로 제거합니다.
   Lab 02에서 제거했더라도 새 에이전트에 다시 추가될 수 있습니다.
   첫 질문 전에 도구 목록을 다시 확인합니다.
5. `prompts/v2.txt`를 VS Code/텍스트 편집기로 열어 지침에 넣습니다.
6. 아래의 **브라우저 출력 형식**도 지침 끝에 추가합니다.

   > 답변 → 적용 날짜와 금액 → 조건/보류 이유 → 근거 문서 ID 순서로 한국어로 설명하세요.
   > 이 브라우저 실습에서는 JSON 대신 사람이 읽을 수 있는 문장으로 답하세요.

7. **Save**로 저장하고 실제 버전 번호를 기록합니다. **Publish**는 이 실습에 필요하지 않습니다.
   아직 외부 시스템을 변경하는 도구나 실제 회사 연결은 추가하지 않습니다.

2026-09-14 실제 화면에서는 **Agents → New agent → Build an agent** 순서로 시작했습니다.
이름을 입력하고 **Create and open playground**를 누른 뒤 생성이 끝날 때까지 기다립니다.
버전 저장과 외부 채널 게시를 같은 작업으로 보지 않습니다.
...
![Luna Prompt Agent의 실제 규정 응답](../assets/live-20260914-action/shots/portal-0262-P07-001-d01-send-screen-change.webp)

![Luna 포털의 실제 근거 부족 답변](../assets/live-20260914-action/shots/portal-0336-P07-001-d05-send-ready.webp)

캡처는 브라우저에서 직접 생성하고 지침·합성 원문을 저장한 실제 agent version 3의 응답입니다.
별도 SDK agent v1, File Search agent v2, IQ 검색과는 구분합니다. [실행 기록](../live-run.md)을 참고하세요.
