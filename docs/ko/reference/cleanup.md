...
**본인 리소스 그룹에서 혼자 학습했나요?** Lab 11 뒤 증거 폴더를 보관하고, [혼자 학습 준비 2단계](../setup-owner.md#self-study)의
리소스 그룹에 이 과정의 리소스만 있다면 그 그룹을 삭제합니다. 프로젝트·배포·Application Insights가 한 번에 삭제됩니다.
**리소스 그룹** 목록에서 사라진 것을 확인한 뒤 `operations-checklist.txt`에 기록합니다. 공유 그룹은 이렇게 삭제하지 않습니다.
