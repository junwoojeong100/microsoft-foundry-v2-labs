...
File Search·하이브리드 RAG·학습자가 직접 하는 Hosted 배포와 대부분의 확장 모듈은 이 모델로 다시 실행하지 않았습니다. 승인된 CI 릴리스와 대화 평가, Agent Optimizer, 안전 제어의 red-team 단계, 릴리스 운영은 이 모델로 다시 실행하거나 확인했습니다([결과](../live-run.md#이전에-실행하지-않은-항목--2026-09-23)).
