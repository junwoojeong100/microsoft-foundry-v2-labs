...
Lab 00의 cloud 사전 확인을 방금 통과했고 같은 터미널의 설정을 바꾸지 않았다면 그 결과로 아래를 확인합니다.
그렇지 않으면 다시 검사합니다.
...
모델이 `json_schema`를 거부하면 중단합니다. 오류를 보존하고 담당자에게 같은 승인된 `gpt-6-sol` 배포의 지원 문제를 해결하도록 요청합니다.
해결되지 않으면 이 단계를 미완료로 기록합니다. 다른 배포·일반 텍스트로 전환하거나 응답을 고치지 않습니다.

<a id="a-terminal-ready"></a>

**B 수강이 아니라 A의 터미널을 준비하러 왔나요?** 이제 터미널 준비가 끝났습니다. 이 SDK 호출로 브라우저 실습까지 완료한 것은 아닙니다.
학습자 ZIP의 설정 카드에서 `준비된 MAF 터미널 위치:`에 이 소스 폴더의 전체 경로를 적습니다.
`7단계 준비 중`이라고 적었다면 실제 경로로 바꾸고, A의 기록은 학습자 ZIP에 유지합니다.
그다음 아래에서 복귀 위치를 고르고 B 경로는 여기서 멈춥니다.

| A에서의 진행 상태 | 다음 위치 |
|---|---|
| A를 시작하기 전 사전 준비 중 | [준비 완료 체크](../setup.md#5-시작-가능-여부)를 마친 뒤 [Lab 00 A](00-start.md#path-a)를 시작합니다. Lab 03의 agent 만들기를 건너뛰지 않습니다 |
