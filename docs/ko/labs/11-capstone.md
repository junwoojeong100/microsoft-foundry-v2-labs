...
| 산출물 | A. 입문 | B. 구현 |
|---|---|---|
| 구조 | 프로젝트·모델·근거·에이전트 관계 그림 | [B 기록에 코드·provider·호출 identity 정리](#b-architecture). 별도 그림은 불필요 |
...
| 실패 검토 | 실제 실패 한 건 또는 전부 통과했다는 기록 | 실제 실패: source run/request/response와 검토 대기 기록. 전체 통과: 검토 메모와 남은 한계 |
...
**인계란 새로 만드는 일이 아니라 이미 한 일을 다른 사람도 이해할 수 있게 정리하는 것입니다.**
나중에 나 또는 다른 학습자가 정확한 에이전트·답변·검토·정리 담당자를 찾을 수 있도록 아래 파일을 함께 보관합니다.
...
실제로 충족한 항목만 체크합니다. 근거가 빠진 항목은 `session-notes.txt`의 **Lab 07 A**에 이유·다음 허용 작업·담당자를 적고,
인계를 **미완료**로 표시합니다. 인증정보 제외 체크는 **어떤 인계에도** 필수입니다.

<a id="explain-your-result"></a>

**저장한 파일로 이해도를 확인합니다. 추가 요청이나 점수를 매기는 시험은 아닙니다.**

| 내 말로 설명할 내용 | 찾아볼 곳 |
|---|---|
| 모델을 바꾸지 않고 규정을 추가하니 무엇이 달라졌나요? | `session-notes.txt`의 Lab 02·03 관찰과 `instructions-baseline.txt` |
| 금액이 맞아도 다른 규정을 인용하면 왜 실패인가요? | 원문 확인 기록과 `assessment-baseline.csv`의 여섯 행 |
| AI가 검토했는데도 왜 예약 승인은 아닌가요? | `workflow-review.txt`의 `approval_status`와 내 검토 |
| 누가 리소스를 중지·삭제하며, 아직 어떤 비용이 남나요? | `operations-checklist.txt` |

설명하기 어렵다면 그 저장 결과와 해당 랩을 다시 읽습니다. 더 좋은 답변을 얻으려고 질문을 다시 보내지 않습니다.
브라우저나 터미널을 닫아도 **Azure 리소스가 삭제되지는 않습니다.** 정리 담당자와 남은 비용을 명확히 남깁니다.

- [ ] 표의 파일이 모두 열리고 완료 기준을 충족합니다.
- [ ] `assessment-baseline.csv`에 업무 실패를 포함한 실제 답변 6행이 모두 있습니다.
- [ ] `workflow-review.txt`에 출력 전체와 본인 검토가 있습니다.
- [ ] `operations-checklist.txt`에 본인 자산마다 중지·삭제 담당자가 있습니다.
- [ ] 폴더에 `.env`·비밀번호·key·token이 없습니다.

**A 완료:** 위 필수 근거 항목을 모두 충족했습니다. 그렇지 않으면 성공적인 실습 완료 대신 **A 미완료 / 인계 기록 완료**로 적습니다.
어느 경우든 기존 폴더를 합의한 수업 경로로 인계하고(혼자 학습하면 보관합니다)
...
**B 구조 설명:** 같은 B 구간의 `Lab 11 구조(코드/명령 경로 / provider / 호출 identity):`를 채웁니다.
기존 Lab 03–08 기록과 Lab 09의 3번을 근거로 실행한 경로마다 한 줄씩 적습니다.
관리형 agent·로컬 MAF·Lab 07 직접 Responses를 구분하고, 확인하지 못한 identity는 미확인으로 표시합니다.
별도 그림이나 새 호출은 필요 없습니다. 이전 양식에 이 칸이 없다면 기존 기록을 교체하지 말고 추가합니다.

**Lab 09 추적 기록:** `operations-checklist.txt`를 열어 Lab 09에서 채운
`실제 추적 근거 또는 조회할 수 없을 때 추적 미확인:` 줄을 확인합니다.
본인의 trace/operation ID와 확인 결과, 또는 `추적 미확인: <이유>`가 있어야 합니다. 이 줄을 채우려고 새 요청을 보내지 않습니다.
...
**B 완료:** [B 근거 목록](#b-evidence)에서 실제 존재하는 파일을 인계합니다. Lab 02의 모델 출력·Lab 03의 관리형 agent 출력·Lab 05의 사람 검토·Lab 09의 추적 상태도 포함합니다.
...
- [ ] 로컬 실행·패키징은 각각 기록하고, 생략한 원격 배포/호출은 미실행으로 표시했다.
- [ ] 본인 정리를 확인했거나 이름을 명시한 권한 있는 담당자에게 인계했고, 공유 자산은 유지했다.

**완료 여부는 위에서 선택한 경로의 기준을 따릅니다.** 평가 완료·전 문항 통과·운영 승인은 서로 다릅니다.
A는 실패를 포함한 실제 dev 답변 6개를 검토하며 holdout을 열지 않습니다.
B는 dev 게이트와 최종 인수/반려 결과 또는 정확한 미완료 단계를 기록합니다.
