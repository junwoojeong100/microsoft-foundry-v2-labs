...
반환 버전과 실제 답변을 적을 `session-notes.txt`를 열어 둡니다. 수동 배포나 **게시** 단계는 필요하지 않습니다.

**Lab 02와 무엇이 달라지나요?** 모델은 그대로입니다. 도우미에게 질문하기 전에 안내 책자를 주듯,
이제 규칙과 가상 정책 문서 6개를 함께 줍니다. 요청에 근거를 추가하는 것이지 **모델 가중치를 학습시키는 과정이 아닙니다.**
문서를 주면 무조건 맞는다고 가정하지 않고 실제 답변을 확인합니다.
...
**상호 작용 모드**는 **텍스트**로 둡니다. 만든 뒤에는 바꿀 수 없습니다.
첫 agent를 열 때 `text-embedding-3-large` 배포가 함께 생성될 수 있습니다. 이에 대한 담당자 승인을 확인한 뒤
**에이전트 만들기 및 플레이그라운드 열기**를 선택합니다. 생성 완료를 기다리고 함께 만들어진 배포는 Lab 09 정리 목록에 적습니다.
...
3. 왼쪽 **지침 입력란 안**을 클릭합니다. **Ctrl+A**(macOS는 **Cmd+A**)로 그 입력란의 텍스트만 선택한 뒤,
...
3. 빠지거나 다른 것이 있으면 위의 입력란 전체 교체를 다시 수행하고 **저장**한 뒤
   `session-notes.txt`의 저장 버전도 새로 반환된 번호로 갱신합니다. 모두 같으면 아무것도 바꾸지 않습니다.
...
2. **지침**을 ZIP의 **`instructions.txt`**로 교체하고 `gpt-6-sol`을 선택합니다. **웹 검색**이 있으면 제거한 뒤 **저장**합니다.
...
기본 경로는 원래 인라인 agent를 계속 사용합니다. [아래 확인 단계로 돌아갑니다](#check-inline-agent).
선택 File Search 결과는 별도로 보관하며, 그 응답을 Lab 03·07의 baseline으로 쓰지 않습니다.
...
**선택 실습의 `-files` agent가 아니라 원래 인라인 agent를 사용합니다.** `session-notes.txt`의 Lab 03 구역에 적은 agent를 엽니다.
**버전**은 기록한 저장 버전과 같고 **저장**은 회색이어야 하며,
**지침**에는 합성 규정 6개를 포함한 `instructions-with-policies.txt` 전체가 있어야 합니다.
이름·버전이 다르거나 미저장 수정이 있다면 보관할 초안은 따로 복사하고, 그 수정 없이 기록한 인라인 버전을 다시 엽니다.
해당 상태로 돌아갈 수 없으면 Lab 03을 미완료로 기록하고 질문 전에 해결합니다.
버전 불일치를 넘기려고 **저장**을 누르지 않습니다.
...
**첫 D01 답변을 읽는 방법:** `TRAVEL-2026`은 문서 ID입니다. 에이전트 버전이나 반드시 눌러야 할 버튼이 아닙니다.
압축을 푼 학습자 폴더에서 **`policies/TRAVEL-2026.txt`**를 엽니다. **Document ID**(문서 ID), **Effective**(적용 기간),
본문의 숙박 한도 순서로 찾습니다. 2026년 9월이 적용 기간에 들어가고 답변이 그 문서의 한도 150000원을 쓰는지 확인합니다.
표현은 달라도 되지만 금액만 맞고 근거가 없으면 충분하지 않습니다. 잘못되거나 빠진 인용은 기록하고,
저장한 답변은 수정하지 않은 채 나머지 질문을 확인합니다.
...
**저장:** 기록한 같은 인라인 agent·버전을 유지합니다. 그 **지침** 입력란 안을 클릭하고 Ctrl+A(macOS는 Cmd+A)로 지침 텍스트만 선택해 복사합니다.
새 일반 텍스트 파일에 붙여 넣고 개인 증거 폴더(압축을 푼 ZIP 폴더)에 `instructions-baseline.txt`로 저장합니다.
`근거 방식 / instructions-baseline.txt 경로:`에는 `인라인 지침`과 이 파일 경로를 적고, 버전은 바로 위 줄에 둡니다.
내려받은 지침 파일만으로 실제 저장 내용을 증명할 수는 없습니다. 이전 회차의 지침 사본은 덮어쓰지 않습니다.
[일반 텍스트 저장 방법](../setup.md#saving-notes)이 필요하면 준비 안내를 확인합니다.

**이 기록을 저장하면 첫 목표에 도달합니다.** 모델·지침·정책 근거를 연결하고 실제 답변 네 개를 확인한 것입니다.
아직 6문항 평가나 A 전체를 마친 것은 아닙니다. 이 에이전트는 Lab 06·07에서도 그대로 사용합니다.
바로 다음 Lab 05는 이 에이전트의 새 버전이 아니라 별도로 준비된 workflow를 사용합니다.
