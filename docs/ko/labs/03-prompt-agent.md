...
![2026-09-23 국문 녹화: New agent → Build an agent](../../assets/g6sol-20260923-ko/screenshots/KP03-002-build-agent-2.webp)
...
![2026-09-23 국문 녹화: 소유 prefix 이름으로 만들고 playground 열기](../../assets/g6sol-20260923-ko/screenshots/KP03-003-name-2.webp)

**화면 확인:** **Agent name**에 본인 접두사를 사용합니다. 녹화의 `mfv2-sol-20260923-ko-policy` 이름을 그대로 쓰지 않습니다.
생성 요청 중 버튼이 비활성화되어 있으면 중복 클릭하지 말고 결과를 기다립니다.
2026-09-23에는 첫 agent를 열 때 계정에 `text-embedding-3-large` 배포도 생성됐습니다. Lab 09에서 소유 자산으로 기록합니다.

#### 모델과 도구 선택

[Lab 02](02-models.md)에서 실제 응답을 확인한 모델 배포를 선택합니다.

![2026-09-23 국문 녹화: 배포 아래 gpt-6-sol 응답 배포 선택](../../assets/g6sol-20260923-ko/screenshots/KP03-004-model-2.webp)
...
![2026-09-23 국문 녹화: 새 agent에서 Web search 제거](../../assets/g6sol-20260923-ko/screenshots/KP03-005-remove-web-2.webp)
...
![2026-09-23 국문 녹화: instructions-with-policies.txt를 Instructions에 붙여 넣기](../../assets/g6sol-20260923-ko/screenshots/KP03-006-instructions-2.webp)
...
![2026-09-23 국문 녹화: 저장하고 반환된 agent 버전 확인](../../assets/g6sol-20260923-ko/screenshots/KP03-007-save-2.webp)

**화면 확인:** 상단 **버전**과 비활성화된 **저장**을 확인합니다.
2026-09-23 녹화는 저장 후 버전 2를 받았지만 본인에게 반환된 버전을 기록해야 합니다.
...
2. ZIP의 **`instructions.txt`**를 Instructions에 붙이고 `gpt-6-sol` 선택·Web Search 제거 후 저장합니다.
...
이 한 응답을 dev 전체 평가 점수로 사용하지 않습니다.

업로드는 **Upload files → Attach files**에서 새 vector index 이름과 파일을 선택합니다.
여기의 vector index는 File Search의 저장소이며 Lab 06의 Azure AI Search
인덱스와 같은 객체가 아닙니다. 업로드의 `Success` 이후 저장소의 6개 파일이 `Completed`인지 확인합니다.
인용 버튼을 누르면 원문이 열린다고 가정하지 말고 파일 이름과 제공된 합성 원문을 대조하세요.
다른 응답이나 검색 제공자로 바꾸지 않습니다. 이 선택 경로는 2026-09-23에 `gpt-6-sol`로 다시 녹화하지 않았습니다.
...
**질문마다 확인·저장한 뒤 다음 질문으로 갑니다.** **New chat**을 누르고 이전 응답이 없어졌는지 확인한 뒤
남은 초안도 바꿉니다. 실제 금액·적용일·원문 ID·승인 조건을 위 표와 대조하고 응답과 판단을 `session-notes.txt`에 저장합니다.
해외 금액을 보류하는 것은 맞지만 `SCOPE-01` 인용이 빠졌다면 그 누락도 기록합니다.
Lab 07과 같은 기준이며 이미 agent가 통과했다는 뜻은 아닙니다.

<details>
<summary>2026-09-23 국문 녹화 화면 — 본인 네 질문의 실행 결과가 아닙니다</summary>

**화면 확인:** 질문을 바꾸기 전에 대화 영역 위의 **New chat**을 누릅니다.
이전 응답이 없어졌는지 확인하고 입력란에 남은 초안도 다음 질문으로 바꿉니다.

![2026-09-23 국문 녹화: D01 · 새 대화, 질문, 실제 답변](../../assets/g6sol-20260923-ko/screenshots/KP03-101-d01-2.webp)

**화면 확인:** 현행 날짜에 150,000원과 `TRAVEL-2026`을 연결하는지 읽습니다.
금액만 맞는지 보지 말고 영수증·승인 조건도 원문과 비교합니다.

![2026-09-23 국문 녹화: D02 · 새 대화, 질문, 실제 답변](../../assets/g6sol-20260923-ko/screenshots/KP03-102-d02-2.webp)

**화면 확인:** 2026년 5월 질문에는 120,000원과 `TRAVEL-2025`가 적용되는지 확인합니다.
오늘 날짜나 최신 문서를 무조건 적용하면 실패로 기록합니다.

![2026-09-23 국문 녹화: D03 · 새 대화, 질문, 실제 답변](../../assets/g6sol-20260923-ko/screenshots/KP03-103-d03-2.webp)

**화면 확인:** 170,000원은 한도 초과이므로 예약 전 승인 절차를 안내해야 합니다.
도우미가 승인됐다고 말하거나 직접 예약했다고 주장하지 않았는지 확인합니다.

![2026-09-23 국문 녹화: D05 · 새 대화, 질문, 실제 답변](../../assets/g6sol-20260923-ko/screenshots/KP03-105-d05-2.webp)
...
![2026-09-23 국문 녹화: 선택 SDK Prompt Agent: 소유 버전 생성](../../assets/g6sol-20260923-ko/screenshots/K03-001-sdk-create-2.webp)
...
2026-09-23에 녹화한 브라우저 agent(버전 2)와 별도 SDK agent(버전 1)의 실제 확인 범위는
