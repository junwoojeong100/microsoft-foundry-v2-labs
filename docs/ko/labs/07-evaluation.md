...
[Lab 03](03-prompt-agent.md#path-a)의 인라인 agent를 열고 저장된 Instructions가 `instructions-baseline.txt`와 일치하는지 확인합니다.
...
1. **`dev-questions.txt`**에서 해당 질문만 복사해 **New chat**에 보냅니다. ID·아래 판정 기준표·평가 열은 보내지 않습니다.
2. **다음 질문 전에** `actual_answer`에 수정하지 않은 실제 답변, `actual_document_ids`에 실제 인용 ID를 저장합니다.
   답변에 빠진 인용을 아래 기준표에서 가져와 채우지 않습니다.
...
**Save** 후 실제 새 버전·변경 이유를 적고, 그 버전에 저장된 Instructions를 **`instructions-candidate.txt`**로 복사합니다.
