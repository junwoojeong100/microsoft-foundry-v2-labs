...
4. `session-notes.txt`의 Lab 06 구간에 답변마다 인용 ID·적용 기간·**맞음** 또는 **틀림**을 적습니다.
   D01은 `현재 정책 ID / 적용일 / 확인 결과:`, D02는 `과거 정책 ID / 적용일 / 확인 결과:`,
   D03은 `한도 초과 승인 근거 / 확인 결과:` 줄에 적습니다.
...
**A 완료:** `session-notes.txt`의 Lab 06 구간에 세 확인 결과가 있고 `선택 IQ Chat의 결과 또는 미선택:`에 `미선택`이 적혀 있습니다
