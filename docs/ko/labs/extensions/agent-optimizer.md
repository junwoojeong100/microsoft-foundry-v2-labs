...
2026-09-25에도 프로젝트에는 지원 optimizer 배포가 없었습니다. 읽기 전용 확인에서 `gpt-5.5` 할당량이 남아 있었지만 할당량만으로 배포 준비 요건을 충족하지는 않습니다.
담당자가 별도로 승인해 지원 optimizer 배포를 준비하고 wizard에서 확인할 때까지 이 모듈은 **미실행**으로 남깁니다.
