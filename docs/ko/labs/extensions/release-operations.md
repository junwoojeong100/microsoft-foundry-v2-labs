...
**준비:** 내 배포 버전, 실제 평가·trace, 모니터링 예산, GitHub OIDC ID.
**완료:** 설정·실제 실행·품질 결정을 구분하고 임시 반복 규칙이 disabled임을 확인함.
**중단:** 기존 배포를 유지하고 누락된 권한/설정을 명시합니다. 성공처럼 보이는 대체 결과를 만들지 않습니다.

## 1. 자동화 전 근거

[Hosted 평가 워크북](../../reference/evaluation-workbook.md)의 실제 버전, 모델 map,
