...
**이번 순서:** A는 1–3절에서 준비된 프로젝트를 확인합니다. 환경이 준비되지 않은 경우에만 담당자가 접힌 담당자 참고를 사용합니다.
...
`session-notes.txt`의 `전체 project endpoint:` 줄을 아래 표의 첫 행과 비교합니다.
...
끝부분 `/api/projects/<project>`가 있어야 합니다. 모양이 다르면 **홈**의 **프로젝트 엔드포인트**를 다시 복사합니다.
