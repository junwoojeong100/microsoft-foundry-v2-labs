...
1. 에이전트의 **평가** 탭에서 **만들기**를 선택합니다. 저장한 **버전**만 선택한 **에이전트**([Lab 07 A 4단계](07-evaluation.md#path-a)의 2번과 같음),
   **개별 턴**, **일회성**을 유지합니다.
2. **데이터**에서 **기존 추적**을 선택합니다. **추적 번호** `15`와 **시간 범위** **7일**을 유지하면 대화가 추적 ID·응답 ID와 함께 표시됩니다.
   마지막 질문 뒤 3–5분 기다립니다.
...
내 에이전트의 **지침**(정책 6개 포함)으로 시작합니다. 2026-09-24 국문 녹화(담당자는 2026-09-23에 그 역할을 할당)의 추적 평가는 세 평가자 모두 15/15였습니다.
에이전트 버전 2의 대화 16개 중 15개이며, 먼저 실행한 선택 Lab 07 A 평가가 에이전트를 호출한 대화 5개도 포함되었습니다. 영문 녹화는 기록한 대화 10개로 10/10이었습니다.
Lab 07 A의 데이터 세트 기반 실행은 질문만 보내 TaskAdherence가 0/6이었습니다. 평가자가 받는 입력이 검증할 수 있는 범위를 정합니다.
...
![2026-09-24 국문 녹화: 에이전트 세부 정보: 이름·저장 버전·모델](../../assets/g6sol-20260924-ko/screenshots/KP09-001-details-2.webp)
