...
승인된 Hosted 릴리스를 다룹니다. 아래의 나머지 확장 결과는 2026-09-16(이전 `gpt-5.6-luna` preset) 기록이며 `gpt-6-sol`의 근거가 아닙니다.
...
| 관리형 prompt agent 핵심 경로 | B | Lab 03 B는 관리형 Prompt Agent를 practitioner 핵심 작업으로 다룸 | 2026-09-24 가이드에 준비. 상태 prepared, not run |
| 함수·로컬 MCP·MAF orchestration | B | 기존 Lab 04–05 | 2026-09-24 `gpt-6-sol` 녹화 |
| A Lab 05 browser option | A | 담당자가 준비한 선택 Hosted workflow agent를 portal Playground에서 테스트 가능 | 2026-09-24 prepared, not run |
...
| Trace 확인을 핵심 근거로 포함 | A/B | Lab 09 A/B는 실제 trace evidence 또는 명시적 unverified reason을 기록 | 2026-09-24 가이드에 준비. 상태 prepared, not run |
| 독립 SDK recipe | B/C | model, prompt-agent, MAF, IQ, Hosted pattern의 최소 offline recipe file | 2026-09-24 offline stub-tested(no Azure calls); live not run |
| SDK pin refresh | B/C | 갱신된 dependency set과 provider constraint 문서화 | 2026-09-24 import contract와 mocked SDK test로 offline-verified. live not run |
...
| [Agent Insights](labs/extensions/agent-insights.md) | C | 반복 trace pattern과 사람 결정 flow를 검토하는 Preview portal 경로 | 2026-09-24 prepared, not run |
...
| Cross-provider model comparison guidance | C | model-operations와 evaluation workbook은 같은 gate 아래 non-OpenAI Foundry Model 하나를 추가하는 법을 설명 | 2026-09-24 prepared, not run |
...
위 표의 날짜가 있는 결과는 과거 기록이며 `gpt-6-sol`의 근거가 아닙니다.
