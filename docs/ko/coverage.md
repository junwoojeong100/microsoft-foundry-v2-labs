...
| 관리형 Prompt Agent를 핵심 경로로 | B | Lab 03 B는 관리형 Prompt Agent를 B의 핵심 작업으로 다룸 | 2026-09-24 갱신한 고정 버전으로 영문·국문 실제 검증(생성, 정확한 버전 호출, 추적). 녹화 없음 |
...
| 추적 확인을 핵심 근거로 | A/B | Lab 09 A/B는 실제 추적 근거 또는 명시적인 미확인 이유를 기록 | 2026-09-24: Application Insights에서 response ID로 관리형 agent 추적을 찾음(영문·국문). 포털 검색은 다시 녹화하지 않음 |
...
| 타사 모델 비교 가이드 | C | 모델 운영과 평가 워크북에서 같은 게이트로 OpenAI 외 Foundry Model 하나를 추가하는 방법 설명 | 미실행: 실습 프로젝트에 OpenAI 외 배포가 없고, 만들려면 담당자 승인이 필요함 |
