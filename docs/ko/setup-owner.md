...
**수업을 준비하나요?** 자습 단계가 아니라 [수업 담당자 체크리스트](#class-owner-checklist)부터 시작합니다.
**환경을 이미 받았나요?** [학습자 준비](setup.md#learner-files)로 돌아갑니다. 프로젝트를 하나 더 만들지 않습니다.
**준비된 환경 없이 혼자 학습하나요?** 본인이 담당자 역할도 맡습니다. [혼자 학습](#self-study)의 A 또는 B 안내를 따릅니다.
...
6. **파일과 설정값.** [준비 2–3절](setup.md#learner-files)을 진행합니다. 학습자 ZIP을 받고, 본인 포털에서 확인한 값으로 설정 카드를 채웁니다.
   ...
   터미널을 준비하기 전까지 `준비된 MAF 터미널 위치:`에는 `7단계 준비 중`을 적습니다. 학습자 ZIP은 코드 환경이 아닙니다.
   ...
   **확인:** **Lab 00 - 설정 카드**에 포털 값이 기록되어 있고, 터미널 위치만 아직 준비 중입니다.
7. **Lab 05 터미널.** [Lab 00 B](labs/00-start.md#path-b) 1–5단계와 [Lab 02 B](labs/02-models.md#path-b) 1–3단계를 마치고
   ...
   **확인:** `doctor --cloud`가 `gpt-6-sol` / `2026-09-22` / `Succeeded`를 보고하고, Lab 02 B가 `model.json`과 `answer-local.json`을 저장했습니다.
   학습자 ZIP의 설정 카드에서 `7단계 준비 중`을 이 소스 폴더의 전체 경로로 바꿉니다.
   Lab 05를 위해 그 폴더·`.venv`·`.env`·저장된 응답을 유지합니다. A의 기록은 B 양식 복사본이 아니라 학습자 ZIP에 계속 적습니다.
...
**학습자에게 인계:** [검증된 환경 설정값](setup.md#environment-card), 중복되지 않는 prefix, A에서 선택한 Lab 05 방식을 전달합니다.
학습자는 [준비 2–4절](setup.md#learner-files)을 마치고 그 페이지의 A 또는 B 링크로 Lab 00을 시작합니다.
아래 내용은 수업에서 선택한 Search/IQ 준비가 있을 때만 이어갑니다.
