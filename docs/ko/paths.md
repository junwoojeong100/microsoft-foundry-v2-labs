...
**A는 Python 작성 없이 브라우저와 준비된 Lab 05 workflow 하나(터미널이 기본이며 담당자 검증을 마친 Hosted Playground는 선택)를 사용하며, B는 코드와 저장된 실행 기록을 다룹니다.**
...
화면은 2026-09-24 국문 녹화와 날짜를 명시한 2026-09-25 확인·보충 자료입니다. 이름·ID·답변은 본인 환경과 다릅니다
([화면 읽는 법](labs/00-start.md#이-가이드의-화면-읽는-법)).
국문 명령은 기본 국문 입력 파일을 사용합니다([언어 규칙](reference/languages.md)).
**아래 시간은 모두 수업 계획값이며 학습자 완료 시간의 실측값이 아닙니다.** 브라우저 응답시간이나 편집 영상 길이로 수업 시간을 검증할 수는 없습니다.
...
권장 운영은 **4시간 세션 두 번**이며 Lab 06까지 첫째 날에 진행하고 둘째 날에는 평가부터 시작합니다.
...
| 세션 | 순서대로 진행할 Lab | 핵심 실습 | 휴식·진행 버퍼 | 합계 |
|---|---|---:|---:|---:|
| Day 1 | 00, 02, 03, 04, 05, 06 | 195분 | 45분 | 240분 |
| Day 2 | 07, 08, 09, 11 | 195분 | 45분 | 240분 |
