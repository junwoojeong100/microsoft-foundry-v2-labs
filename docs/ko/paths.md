...
**[준비 카드](setup.md)에서 시작하고 자기 경로의 다음 링크만 따라갑니다.**
A는 학습자 ZIP, B는 소스 복사본과 그 안의 기록 양식을 사용합니다.
시작 카드 → 조작·명령 → 완료 기준 순서로 진행하세요.
이미지와 접힌 녹화 갤러리는 선택 참고 자료이며 본인 실행 결과가 아닙니다.
2026-09-23 `gpt-6-sol` 국문 녹화를 사용합니다.
국문과 영문은 별도 실행·별도 원본이며 본인의 값과 결과는 따로 확인합니다.
[국문 녹화 영상](video-summary.md)과 [액션 인덱스](action-captures.md)를 제공합니다.
